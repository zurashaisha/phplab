...
if(isset($_POST["query"]))
{
	...
	if($_POST["query"] != '')
	{
		...
		$query = "
		SELECT dev_name, ip, devserial, dev_role 
		FROM devices 
		WHERE dev_name LIKE :dev_name 
		OR ip LIKE :ip
		OR devserial LIKE :devserial
		OR dev_role LIKE :dev_role
		ORDER BY id DESC
		";
		...
		{
			$data[] = array(
				'dev_name'		=>	str_ireplace($replace_array_1, $replace_array_2, $row["dev_name"]),
				'ip'	=>	str_ireplace($replace_array_1, $replace_array_2, $row["ip"]),
				'devserial'	=>	str_ireplace($replace_array_1, $replace_array_2, $row["devserial"]),
                'dev_role'	=>	str_ireplace($replace_array_1, $replace_array_2, $row["dev_role"])
			);
		}
